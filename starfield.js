...
function draw(now) {
  ...
  const dim = Math.max(0, 1 - (now - dimStart) / 250) * 0.17;
  ...
  for (const star of stars) {
    ...
    const twinkle = reducedMotion ? 1 : 0.82 + Math.sin(elapsed * (1.4 + depth * 0.28) + star.phase) * 0.18;
...
document.addEventListener('visibilitychange', () => {
  ...
  else { lastTime = performance.now(); frame = requestAnimationFrame(animate); }
});
